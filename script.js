...
const musicBtnCover=document.getElementById('musicBtnCover');
...
function imageExists(img){
  return !!img.getAttribute('src') && !img.getAttribute('src').endsWith('/photo7.jpg?missing') && !img.getAttribute('src').endsWith('/photo8.jpg?missing');
}
function updateVisiblePages(){
  optionalSpreads.forEach(spread=>spread.style.display='');
  const missing=optionalPhotos.filter(img=>!imageExists(img)).length;
  if(missing>=2){ optionalSpreads.forEach(spread=>spread.style.display='none'); }
...
function startMusic(){
  bgMusic.volume=0.18;
  bgMusic.play().then(()=>{musicEnabled=true;syncMusicButtons();}).catch(()=>{musicEnabled=false;syncMusicButtons();});
}
function toggleMusic(){
  if(bgMusic.paused){ bgMusic.play(); musicEnabled=true; }
  else { bgMusic.pause(); musicEnabled=false; }
  syncMusicButtons();
}
function syncMusicButtons(){
  musicBtn.textContent=musicEnabled?'♫':'♪';
  musicBtnCover.textContent=musicEnabled?'♫ Music On':'♫ Music Off';
}
...
musicBtnCover.addEventListener('click',()=>{ if(!musicEnabled) startMusic(); else toggleMusic(); });
...
function revealLetterLines(){
  ...
  lines.forEach((line,i)=>setTimeout(()=>line.classList.add('reveal'),250+i*450));
}
function openLetter(){
